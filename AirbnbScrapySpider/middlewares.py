...
class AirbnbScrapySpiderDownloaderMiddleware:
    ...
    def process_request(self, request, spider):
        ...
        if request.meta.get('flag') == flags['room_detail']:
            ...
            WebDriverWait(spider.browser, 8, 1).until(
                EC.presence_of_element_located((By.XPATH, "/html/body/div[3]/div/main/div[2]/div/div/div/div[2]/div/div[1]/div/main/div[4]/div/div/div/section/div[1]/h1/div"))
            )
            ...
            spider.browser.switch_to_window(spider.browser.window_handles[0])

        elif request.meta.get('flag') == flags['homes_list']:
            # 这里则是获取房源列表的操作
            # 获取本页的列表
            element = spider.browser.find_element_by_tag_name('body')
            element.send_keys(Keys.END)
